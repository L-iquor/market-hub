...
const BASE_TEMPLATES = {
  A: {
    ...
    desc: '问题解决型写作顺序（自动注入）',
    body: '写作顺序：具体喝酒痛苦场景（辣嗓子像吞刀片/涨肚想吐/第二天头疼）→ 用「直到」引出每天烈刻 → 逐点解决（10度=不高度，苹果发酵=顺滑不辣，0糖0卡=无负担）→ 茅台产区10年基酒质感背书 → 适用场景+推荐特定痛点人群尝试\n标题：3个，含emoji，体现「从痛苦到解脱」的具体反差感\n评论话术：3条，覆盖「引发共鸣 / 问购买渠道 / 打消成分疑虑」\n品牌事实：至少覆盖 10度 / 0糖0卡 / 茅台产区 / 苹果发酵 中3项',
    example: `备选标题：
...
③ 0糖0卡是真的吗？配料表真的很干净，放心冲！`,
  ...
  B: {
    ...
    desc: '场景种草型写作顺序（自动注入）',
    body: '写作顺序：「第一次喝就被惊艳」的真实感（打破刻板印象）→ 简述啤酒/白酒痛点（1-2句，点到即止）→ 感官体验（茅台底蕴+苹果发酵气泡炸开→像香槟顺滑，口味感官描写）→ 0糖0卡无负罪感自然带出 → 具体生活场景（追剧/闺蜜下午茶/加冰氛围感）→ 推荐平时不喝白酒的朋友\n标题：3个，含emoji，突出「颠覆认知」或「中国香槟/液体蛋糕」等品类新定义词\n图片建议：9张（封面+倒酒/原料/标签/口味/氛围/餐搭/手持）\n评论话术：3条，覆盖「讨论口味/催库存/打消白酒味顾虑」',
    example: `备选标题：
...
③ 很多人问会不会有白酒味，放心，果香很足，只有一点点回甘有酒香，完全不辣！`,
  ...
  C: {
    ...
    desc: '对比评测型写作顺序（自动注入）',
    body: '写作顺序：市场果酒/气泡酒良莠不齐引发「扒配料表」动机 → 基酒拆解（茅台产区糯高粱+10年藏酒 vs 普通食用酒精）→ 工艺解析（北纬37度红心苹果发酵融合 ≠ 勾兑）→ 竞品对比（糖分/口感横向，品牌模糊处理用「某R牌/某M牌」）→ 0糖0卡+10度口感数据 → 性价比+口感双赢结论\n标题：3个，含emoji，含「测评/扒配料/横评/硬核」等词，体现专业感\n评论话术：3条，覆盖「认同测评/分享使用经验/打消智商税顾虑」\n禁止：不要编造实验室数据、真实竞品品牌、销量数字',
    example: `备选标题：
...
③ 这种硬核测评多来点，避雷了好多网红酒！`,
  ...
  D: {
    ...
    desc: '教程干货型写作顺序（自动注入）',
    body: '写作顺序：引发好奇（明星私厨/高端局都在用，很多人不知道怎么喝）→ 第一步加冰（最佳6-8度，苹果香收敛→茅台松香透出，层次感）→ 第二步选杯（笛形杯/郁金香杯，聚住气泡→每口爆珠感）→ 第三步餐搭（青提+海鲜刺身/菠萝+辣味火锅，0糖解辣解腻）→ 品类科普（真白酒 vs 预调酒本质区别）→ 社交货币收尾（学会这几招=聚会最懂酒的行家）\n标题：3个，含emoji，含「神仙喝法/小白攻略/干货/揭秘」等词\n评论话术：3条，覆盖「实践反馈确认/分享行动计划/引发收藏欲」\n收藏价值：每个干货点必须实用，步骤必须有「为什么」',
    example: `备选标题：
